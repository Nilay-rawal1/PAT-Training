...
public:
    ...
    int KthSmallestElement(Node *root, int K) {
        ...
        int result = -1; // Initialize with an invalid value

        // Helper function to perform inorder traversal and find Kth smallest
...
private:
    ...
    void inorder(Node *node, int K, int &count, int &result) {
        // Base case: if node is NULL or Kth smallest element already found
        ...
        // If count is equal to K, we found the Kth smallest element
        if (count == K) {
            result = node->data;
            return;
        }
